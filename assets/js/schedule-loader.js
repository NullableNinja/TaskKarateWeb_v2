 ...
 * Ensures exact data parity with original HTML structure
 */
...
(function() {
  ...
  function populateKidsSchedule(data) {
    const tbody = document.querySelector('#kids-schedule tbody');
    if (!tbody) return;
  ...
  function populateTeensAdultsSchedule(data) {
    const tbody = document.querySelector('#teens-adults-schedule tbody');
    if (!tbody) return;
  ...
  function populateSparringSchedule(data) {
    const tbody = document.querySelector('#sparring-schedule tbody');
    if (!tbody) return;
  ...
  function populateWeaponsSchedule(data) {
    const tbody = document.querySelector('#weapons-schedule tbody');
    if (!tbody) return;
  ...
  function populateSpecialSchedules(data) {
    const tbody = document.querySelector('#special-schedule tbody');
    if (!tbody) return;
  ...
  function populateEskrimaSchedule(data) {
    const tbody = document.querySelector('#eskrima-schedule tbody');
    if (!tbody) return;
  ...
  function createScheduleRow(group, program, allowSpecialNote = false) {
    ...
    beltCell.className = 'belt-cell';
    group.belts.forEach(belt => {
      const beltDot = document.createElement('span');
      beltDot.className = `belt-dot belt-dot--${belt}`;
      beltDot.setAttribute('aria-label', `${capitalize(belt)} Belt`);
      beltCell.appendChild(beltDot);
    });
